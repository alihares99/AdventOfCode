...
auto get_input(const string& filename) {
    ...
    return make_tuple(wires, gates, int(wires.size() / 2));
}

uint64_t part1(unordered_map<string, bool>& wires, const vector<LogicGate>& gates) {
    int n_z = int(wires.size()) / 2 + 1;
...
string part2(vector<LogicGate>& gates, int n_bits) {
...
int main() {
    auto [wires, gates, n_bits] = get_input("input.txt");
    cout << part1(wires, gates) << endl;
    cout << part2(gates, n_bits) << endl;
    return 0;
}