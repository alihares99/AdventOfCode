...
struct Day20 : public Solution {
    ...
    void get_input(const string& filename) {
        ...
        maze_p = make_unique<Maze>(move(lines));
    }
    ...
    string part1(const string& filename) override {
        ...
        vector<int> cheats;
        for (auto it = maze_p->begin(); it.is_valid(); ++it) {
            auto p = it;
            for (int i = 0; i < 3; i++) {
                auto tmp = p;
                if (i == 1) tmp.p.turn_left();
        ...
        ranges::sort(cheats);
        auto it = lower_bound(cheats.begin(), cheats.end(), 100);
        return to_string(cheats.end() - it);
    }
    ...
    string part2(const string& filename) override {
        ...
        for (auto it = maze_p->begin(); it.is_valid(); ++it) {
            ...
            int i_end = min(it.p.i + 20, maze_p->n_row - 1);
            for (int i = i_start; i <= i_end; i++) {
                int j_start = max(it.p.j - (20 - abs(i - it.p.i)), 0);
                int j_end = min(it.p.j + (20 - abs(i - it.p.i)), maze_p->n_col - 1);
